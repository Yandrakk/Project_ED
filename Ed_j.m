...
Tau = f_lambda.*((1-0.05)*e*Lp_plus*a_lambda)./(-1+(a_lambda*Lp_plus).^2); % R = 0.05, black material
...
J_Wa_x3_lambda = e*f_lambda.*(1-0.05).*exp(-a_lambda*Wn_plus).*(1-exp(-a_lambda*vpa(Wa))).*sech(Wn/Lp);
J_Wa_x3_lambda_1 = e*f_lambda.*(1-0.05).*exp(-a_lambda*Wn_plus_1).*(1-exp(-a_lambda*vpa(Wa))).*sech(Wn/Lp);
J_Wa_x3_lambda_2 = e*f_lambda.*(1-0.05).*exp(-a_lambda*Wn_plus_2).*(1-exp(-a_lambda*vpa(Wa))).*sech(Wn/Lp);
J_Wa_x3_lambda_3 = e*f_lambda.*(1-0.05).*exp(-a_lambda*Wn_plus_3).*(1-exp(-a_lambda*vpa(Wa))).*sech(Wn/Lp);
J_Wa_x3_lambda_4 = e*f_lambda.*(1-0.05).*exp(-a_lambda*Wn_plus_4).*(1-exp(-a_lambda*vpa(Wa))).*sech(Wn/Lp);%photocurrent of space charge zone (n-n+ zone)
...
J_Wd_lambda = e .* f_lambda.*(1-0.05).*exp(-a_lambda*Wn).*(1-exp(-a_lambda*Wd)); %current of depletion zone 
...
J_Wa_x3_Wn = e*f(lambda_fixe)*(1-0.05)*exp(-a(lambda_fixe)*Wn_plus)*(1-exp(-a(lambda_fixe)*vpa(Wa)))*sech(Wn_range/Lp);
...
J_Wd = e * f(lambda_fixe)*(1-0.05)*exp(-a(lambda_fixe)*Wn_range)*(1-exp(-a(lambda_fixe)*Wd));
J_tot = J_p_Wn + J_n_Wn  + Jn0_plus_x1_Wn + J_Wa_x3_Wn + J_Wd ;
figure;
plot(Wn_range, J_n_Wn,'bo', Wn_range, J_p_Wn, 'go',Wn_range, J_Wd, 'ro',Wn_range, Jn0_plus_x1_Wn,'yo', Wn_range, J_Wa_x3_Wn,'ko' );
legend('n layer','p layer',' depletion zone','n+ layer','space charge region');
xlabel('Wn (m)');
ylabel ('Photocurrent A/m2');
title('Contribution to the photocurrent density VS thickness of n layer');
figure;
