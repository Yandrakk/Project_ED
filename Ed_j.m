...
J_p_lambda = -k1_lambda.*Lp.*a_lambda.*exp(-a_lambda.*Wn) + (k1_lambda./((Sen_plus_n_lambda.*Lp/Dp).*sinh(Wn/Lp)+cosh(Wn/Lp))).*((Sen_plus_n_lambda.*Lp/Dp)+ a_lambda.*Lp - exp(-a_lambda.*Wn).*((Sen_plus_n_lambda*Lp/Dp).*cosh(Wn/Lp) + sinh(Wn/Lp))); % photocurrent generated by p layer calculated at x3
...
J_Wd_lambda = e .* f_lambda.*(1-0.05).*exp(-a_lambda*Wn).*(1-exp(-a_lambda*Wd));
J_tot_lambda = Jn0_plus_x1_lambda + J_Wa_x3_lambda +J_p_lambda +J_n_lambda +J_Wd_lambda;
 figure;
 plot(lambda, J_tot_lambda, 'm-');
 ...
 legend('contribution n region','contribution p region','contribution depletion zone', 'contribution over doped zone', 'contribution Wa');
  ylabel('Photocurrent density A/m2');
 xlabel('wavelength (m)');
